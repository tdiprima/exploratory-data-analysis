...
df["visit_duration_days"] = (
    df["visit_end_datetime"] - df["visit_start_datetime"]
).dt.days
...
with open("eda_summary_report.txt", "w") as f:
    ...
    f.write(
        f"Date Range: {df['visit_start_datetime'].min()} to {df['visit_start_datetime'].max()}\n"
    )
    ...
    f.write(
        f"Average Visits per Patient: {df.shape[0] / df['person_id'].nunique():.2f}\n"
    )
